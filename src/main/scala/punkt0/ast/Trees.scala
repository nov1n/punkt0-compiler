...
object Trees {
  ...
  case class ClassDecl(id: Identifier, parent: Option[Identifier], vars: List[VarDecl], var methods: List[MethodDecl], foreignPath : Option[String]) extends Tree with Symbolic[ClassSymbol]
  case class VarDecl(tpe: TypeTree, id: Identifier, expr: ExprTree) extends Tree with Symbolic[VariableSymbol]
  case class MethodDecl(overrides: Boolean, retType: TypeTree, id: Identifier, args: List[Formal], vars: List[VarDecl], exprs: List[ExprTree], retExpr: ExprTree, foreignPath : Option[String]) extends Tree with Symbolic[MethodSymbol] {
  }
